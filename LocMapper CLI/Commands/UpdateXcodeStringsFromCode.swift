...
#if canImport(os)
					LocMapperConfig.oslog.flatMap{ os_log("Storyboard or xib “%{public}@” that is marked as unlocalized does not exist.", log: $0, type: .default, unlocalizedXibsPath) }
#endif
					LocMapperConfig.logger?.warning("Storyboard or xib that is marked as unlocalized does not exist.", metadata: ["path": "\(unlocalizedXibsPath)"])
				}
			}
		}
		...
		if !skipCode, let localizablesPath = localizablesPath {
			...
			for language in languages {
				...
				for unusedStringsfilesPath in unusedStringsfilesPaths {
					if (genstringsStringsfiles.contains{ $0.key == unusedStringsfilesPath }) {
#if canImport(os)
						LocMapperConfig.oslog.flatMap{ os_log("Strings file “%{public}@” that is marked as unused has been generated by genstrings (language is %{public}@).", log: $0, type: .default, unusedStringsfilesPath, language) }
#endif
						LocMapperConfig.logger?.warning("Strings file that is marked as unused has been generated by genstrings.", metadata: ["path": "\(unusedStringsfilesPath)", "language": "\(language)"])
					} else if !(foundStringsfile.contains(unusedStringsfilesPath)) {
#if canImport(os)
						LocMapperConfig.oslog.flatMap{ os_log("Strings file “%{public}@” that is marked as unused does not exist (language is %{public}@).", log: $0, type: .default, unusedStringsfilesPath, language) }
#endif
						LocMapperConfig.logger?.warning("Strings file that is marked as unused does not exist.", metadata: ["path": "\(unusedStringsfilesPath)", "language": "\(language)"])
					}
				}
			}
		}
